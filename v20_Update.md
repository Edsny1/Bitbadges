# BitBadges v20 Upgrade Hazırlık Rehberi
...
```bash
cp build/bitbadgeschain-linux-amd64 $HOME/.bitbadgeschain/cosmovisor/upgrades/v20/bin/bitbadgeschaind
...
ls -la $HOME/.bitbadgeschain/cosmovisor/upgrades/v20/bin/

# Binary'nin çalıştırılabilir olduğunu doğrulayın
$HOME/.bitbadgeschain/cosmovisor/upgrades/v20/bin/bitbadgeschaind version --long
```
...
```
error during handshake: error on replay: UPGRADE "v20" NEEDED at height: XXXXX
```
...
ln -s $HOME/.bitbadgeschain/cosmovisor/upgrades/v20 $HOME/.bitbadgeschain/cosmovisor/current
```
...
git clone https://github.com/BitBadges/bitbadgeschain.git
cd bitbadgeschain && git checkout v20 && make build-linux/amd64
mkdir -p $HOME/.bitbadgeschain/cosmovisor/upgrades/v20/bin
cp build/bitbadgeschain-linux-amd64 $HOME/.bitbadgeschain/cosmovisor/upgrades/v20/bin/bitbadgeschaind
chmod +x $HOME/.bitbadgeschain/cosmovisor/upgrades/v20/bin/bitbadgeschaind
$HOME/.bitbadgeschain/cosmovisor/upgrades/v20/bin/bitbadgeschaind version
...
```bash
cat > $HOME/v20_upgrade_monitor.sh << 'EOF'
#!/bin/bash

UPGRADE_HEIGHT=XXXXX  # v20 upgrade bloğunu buraya yazın
TARGET_VERSION="v20"
...
while true; do
    ...
    if [ "$CURRENT_HEIGHT" -ge $((UPGRADE_HEIGHT - 100)) ] && [ "$CURRENT_HEIGHT" -lt "$UPGRADE_HEIGHT" ]; then
        ...
        echo "⚠️  WARNING: v20 Upgrade in $BLOCKS_LEFT blocks!"
...
chmod +x $HOME/v20_upgrade_monitor.sh
```

**Kullanımı:**
```bash
# Tmux içinde çalıştırın
tmux new -s v20_monitor
./v20_upgrade_monitor.sh
...
cp $HOME/bitbadgeschain/build/bitbadgeschain-linux-amd64 $HOME/.bitbadgeschain/cosmovisor/upgrades/v20/bin/bitbadgeschaind
chmod +x $HOME/.bitbadgeschain/cosmovisor/upgrades/v20/bin/bitbadgeschaind
```
...
- **Docs:** [BitBadges Documentation](https://docs.bitbadges.io)

---

**Son Güncelleme:** 26 Kasım 2024

**Not:** Bu rehber v17, v18 ve v19 upgrade'lerinde yaşanan sorunlar göz önünde bulundurularak hazırlanmıştır. Cosmovisor'ın otomatik geçiş yapmaması durumu için manuel adımlar eklenmiştir.
