# BitBadges v22 Upgrade Hazırlık Rehberi
...
## 📋 v22 Upgrade Bilgileri

- **Upgrade Adı:** v22
- **Upgrade Blok Yüksekliği:** 8030000
- **Tahmini Zaman:** 9 Ocak 2026, 11:27:42 AM EST (18:27:42 Türkiye Saati)
- **Oylama Dönemi:** Şimdi + 24 saat
- **Mevcut Versiyon:** v21
- **Hedef Versiyon:** v22
- **Release Bilgileri:** https://github.com/BitBadges/bitbadgeschain/releases/tag/v22
...
```bash
cp build/bitbadgeschain-linux-amd64 $HOME/.bitbadgeschain/cosmovisor/upgrades/v22/bin/bitbadgeschaind
...
ls -la $HOME/.bitbadgeschain/cosmovisor/upgrades/v22/bin/

# Binary'nin çalıştırılabilir olduğunu doğrulayın
$HOME/.bitbadgeschain/cosmovisor/upgrades/v22/bin/bitbadgeschaind version --long
```
...
```
error during handshake: error on replay: UPGRADE "v22" NEEDED at height: 8030000
```
...
ln -s $HOME/.bitbadgeschain/cosmovisor/upgrades/v22 $HOME/.bitbadgeschain/cosmovisor/current
```
...
git clone https://github.com/BitBadges/bitbadgeschain.git
cd bitbadgeschain && git checkout v22 && make build-linux/amd64
mkdir -p $HOME/.bitbadgeschain/cosmovisor/upgrades/v22/bin
cp build/bitbadgeschain-linux-amd64 $HOME/.bitbadgeschain/cosmovisor/upgrades/v22/bin/bitbadgeschaind
chmod +x $HOME/.bitbadgeschain/cosmovisor/upgrades/v22/bin/bitbadgeschaind
$HOME/.bitbadgeschain/cosmovisor/upgrades/v22/bin/bitbadgeschaind version
...
```bash
cat > $HOME/v22_upgrade_monitor.sh << 'EOF'
#!/bin/bash

UPGRADE_HEIGHT=8030000
TARGET_VERSION="v22"
...
while true; do
    ...
    if [ "$CURRENT_HEIGHT" -ge $((UPGRADE_HEIGHT - 100)) ] && [ "$CURRENT_HEIGHT" -lt "$UPGRADE_HEIGHT" ]; then
        ...
        echo "⚠️  WARNING: v22 Upgrade in $BLOCKS_LEFT blocks!"
...
chmod +x $HOME/v22_upgrade_monitor.sh

# Tmux içinde çalıştırın
tmux new -s v22_monitor
./v22_upgrade_monitor.sh
...
mkdir -p $HOME/bitbadges_backup_v22
cp $HOME/.bitbadgeschain/config/priv_validator_key.json $HOME/bitbadges_backup_v22/
cp $HOME/.bitbadgeschain/config/node_key.json $HOME/bitbadges_backup_v22/
cp $HOME/.bitbadgeschain/data/priv_validator_state.json $HOME/bitbadges_backup_v22/
```
...
echo "=== v22 Binary Ready ==="
$HOME/.bitbadgeschain/cosmovisor/upgrades/v22/bin/bitbadgeschaind version

echo "=== Blocks Until Upgrade ==="
CURRENT=$(bitbadgeschaind status 2>&1 | jq -r '.SyncInfo.latest_block_height')
echo "Current: $CURRENT | Target: 8030000 | Remaining: $((8030000 - CURRENT))"
```
...
**Not:** Bu rehber, v21'den v22'ye geçiş için hazırlanmıştır. Tüm adımları dikkatlice takip edin ve upgrade bloğuna ulaşmadan önce hazırlıklarınızı tamamlayın.
